...
<#assign form_field_dictCode="">
	...
	<#elseif po.dictField?default("")?trim?length gt 1>
        ...
        <a-form-item label="$t(${po.filed})" :labelCol="labelCol" :wrapperCol="wrapperCol">
	<#if po.fieldDbName == tableVo.extendParams.pidField>
		<#assign form_tree_select = true>
		<#assign pidFieldName = po.fieldName>
          <j-tree-select
            ref="treeSelect"
            placeholder="$t(${po.filed})"
	...
	<#elseif po.classType =='date'>
		<#assign form_date=true>
          <j-date placeholder="请选择$t(${po.filed})" v-decorator="['${po.fieldName}'${autoWriteRules(po)}]" :trigger-change="true" style="width: 100%" <#if po.readonly=='Y'>disabled</#if>/>
	<#elseif po.classType =='datetime'>
		<#assign form_date=true>
          <j-date placeholder="请选择$t(${po.filed})" v-decorator="['${po.fieldName}'${autoWriteRules(po)}]" :trigger-change="true" :show-time="true" date-format="YYYY-MM-DD HH:mm:ss" style="width: 100%" <#if po.readonly=='Y'>disabled</#if>/>
