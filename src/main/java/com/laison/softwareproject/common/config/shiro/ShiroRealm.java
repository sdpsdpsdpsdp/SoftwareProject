...
public class ShiroRealm extends AuthorizingRealm {
    ...
    public SysUser checkUserTokenIsEffect(String token) throws AuthenticationException {
        ...
        SysUser SysUser = TokenUtils.getSysUser(username, commonApi, redisUtil);
        //SysUser SysUser = commonApi.getUserByName(username);
        if (SysUser == null) {
        ...
        String userTenantIds = SysUser.getRelTenantIds();
        if(oConvertUtils.isNotEmpty(userTenantIds)){
            ...
            if(oConvertUtils.isNotEmpty(contextTenantId) && !str.equals(contextTenantId)){
                ...
                if(!oConvertUtils.isIn(contextTenantId, arr)){
                    ...
                    SysUser SysUserFromDb = commonApi.getUserByName(username);
                    if (oConvertUtils.isNotEmpty(SysUserFromDb.getRelTenantIds())) {
                        String[] newArray = SysUserFromDb.getRelTenantIds().split(",");
                        if (oConvertUtils.isIn(contextTenantId, newArray)) { 
                            isAuthorization = true;

                            //清空redis缓存
                            String SysUserKey = CacheConstant.SYS_USERS_CACHE + "::" + username;
                            redisUtil.del(SysUserKey);
                        }
                    }
                    //========================================================================
